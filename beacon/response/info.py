...
async def ejp_handler(request: Request):
    LOG.info('Running a GET info request')

    # Fetch datasets info
    json_body = await request.json() if request.method == "POST" and request.has_body and request.can_read_body else {}
    qparams = RequestParams(**json_body).from_request(request)
    _, _, datasets = get_ejp_datasets(None, qparams)
    beacon_datasets = [r for r in datasets]
...
async def epnd_handler(request: Request):
    LOG.info('Running a GET info request')

    # Fetch datasets info
    json_body = await request.json() if request.method == "POST" and request.has_body and request.can_read_body else {}
    qparams = RequestParams(**json_body).from_request(request)
    _, _, datasets = get_epnd_datasets(None, qparams)
    beacon_datasets = [r for r in datasets]

    # all_datasets = [r['name'] for r in beacon_datasets]
    ...
    response_converted = build_epnd_beacon_info_response(beacon_datasets,
                                                         qparams,
                                                         lambda x, y, z: x,
                                                         [])
    return await json_stream(request, response_converted)
